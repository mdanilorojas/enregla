    ...
    <Card padding="none">
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-amber-100 text-amber-600 flex items-center justify-center shadow-sm shadow-amber-500/10">
            <CalendarClock size={16} strokeWidth={1.8} />
          </div>
          <div>
            <span className="text-[14px] font-semibold text-gray-900 block leading-tight">Próximos vencimientos</span>
            <span className="text-[11px] text-gray-400 font-medium">{upcoming.length} pendientes</span>
          </div>
        </div>
        <button
        ...
        {upcoming.map((item) => (
          <div key={item.id} className="flex items-center gap-3.5 px-5 py-3.5 hover:bg-gray-50/50 transition-colors group">
            ...
            <div className="text-right shrink-0">
              <span className={`text-[12px] font-semibold whitespace-nowrap block ${
